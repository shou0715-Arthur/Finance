...
```text
dist\AIStockAnalysis\config\api-key.txt
```
...
首次使用時，`config\api-key.txt` 會是空白範本。請在 GUI 內輸入 FinMind / Gemini key 後按「儲存 Key」，程式會寫入同一個檔案，下次執行會自動載入。請不要把這個檔案上傳到 GitHub。
...
5. 建立 Gemini API key。
6. 複製 API key。
7. 貼到 `config\api-key.txt`：

```text
GEMINI_API_KEY=你的_Gemini_API_key
```

Google 官方文件建議使用環境變數保存 key；本程式則使用本機 `config\api-key.txt`，請不要把該檔案上傳到 GitHub。
...
Python 版執行時，位置是專案資料夾內的 `config\watchlist.json`。

`.exe` 版執行時，位置是：

```text
dist\AIStockAnalysis\config\watchlist.json
```
...
代表 `FINMIND_API_KEY` 不正確、過期，或 `config\api-key.txt` 仍是範例文字。
...
- 不要把 `config\api-key.txt` 上傳到 GitHub。
- 不要在截圖中顯示完整 API key。
